# Editing phases 1–10 — delivery log

Moved out of `AGENTS.md`, where ~360 lines of dated changelog sat between the build commands and
the coding conventions and buried both. It is a record of what each phase of the editing work
delivered, not guidance, so it belongs with the other plans.

Status as recorded: 2026-07-11.
...
- `examples/editing_p1/{main.cpp,Makefile}`: Test driver.
- `AGENTS.md`: This file.
...
- `examples/edit_demo/main.cpp` — switched to `flushSceneUpdates`.
- `examples/editing_p1/main.cpp` — 8 new P5 dirty-flag assertions (all pass).
...
- `examples/editing_p1/main.cpp`: 21 P6 assertions (names, paths, transforms, world matrix, voxel counts, listComponents, transform mutation + restore). Skip Asset kinds in `findComponents`.
- `examples/edit_demo/main.cpp`: Skip Asset kinds in component search.
- `examples/editing_p1/Makefile`: Link `projectV-scene_query`.
...
- `tests/manual/edit_demo/main.cpp` & `examples/30-renderers/main.cpp`:
...
- `examples/SceneEditor/`: dockable editor shell — Viewport / Scene Hierarchy / Inspector /
