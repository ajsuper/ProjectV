// =============================================================================
// pjv_cascade_ws.sc  --  WORLD-SPACE radiance-cascade gather + merge (renderer 6).
//
// Same radiance-cascade structure as pjv_cascade.sc (full-res atlas, probe = Tc x Tc
// direction tile, top-down merge, identical intervals and octahedral directions) but the
// gather traces a REAL voxel-DDA ray through the scene instead of marching the depth
// buffer. That is the whole point of the world-space variant: a screen-space ray can't
// see off-screen geometry, so it can't tell "blocked" from "sees sky" and defaults to
// sky -> a flat sky over everything. A world-space ray either hits a voxel (correctly
// occluded, or picks up that voxel's bounced sunlight) or escapes the scene entirely to
// the true sky. So sky occlusion / sky shadows come out right.
//
...
//   SAMPLER2D gPos, gNormal (+ upperCascade unless IS_TOP)
//   #include <pjv_utils_DDA.sc>   (Ray/RayQuery/raySceneIntersect/fetchVoxelData + scene)
...
// low occluders / softening contact darkening. 0 falls back to NORMAL_BIAS. Renderer 5 passes vs=0 so
// it is unaffected (keeps NORMAL_BIAS).
...
// vs = probe's voxel edge size (0 disables the grazing-relief skip; renderer 5 passes 0).
vec4 gatherRayWS(vec3 P, vec3 N, vec3 dir, int c, float vs) {
    ...
    // a per-hit test. vs = 0 (renderer 5) -> just NORMAL_BIAS.
...
void upperProbeSurface(ivec2 upProbe, int upS, out vec3 sp, out vec3 sn, out float sv) {
    vec2 uv = (vec2(upProbe) + 0.5) * float(upS) / windowRes.xy;
...
vec3 mergeUpper(ivec2 probe, ivec2 tile, int c, vec3 P, vec3 N) {
    ...
    ivec2 upProbesN = ivec2(windowRes.xy) / upS;
...
vec3 computeCascadeTexel(ivec2 fragPx) {
    ...
    // This cascade fills only the (probesN * Dc) sub-rectangle at the texture origin. Skip every
    // pixel outside it BEFORE the costly DDA gather -- that skip is the whole point: unused texels
    // cost ~nothing, so the cascade's gather work shrinks to (Dc/sc)^2 of full-res.
    ivec2 probesN  = ivec2(windowRes.xy) / sc;
    ...
    // Probe's world anchor: map the (sparse) probe grid back onto the full-res G-buffer.
    vec2 probeUV = (vec2(probe) + 0.5) * float(sc) / windowRes.xy;
...
#ifdef FACE_PROBES
    // Per-face probe anchoring (renderer 6). The gather origin is the voxel FACE CENTRE read from
    // the gbuffer (gFace, xyz = face centre, already quantized to the voxel grid and lying on the
    // surface plane). Because the face centre is camera-independent and identical for every screen
    // probe that lands on the same face, all those probes gather from ONE stable world origin ->
    // effectively one probe per voxel face, rock-steady across frames (no screen-drift jitter, no
    // world-grid snap needed -- the voxelisation already IS the grid). This is what lets the probe
    // spacing be coarse (far fewer gather rays) while the per-face temporal accumulate stays clean.
    vec4 gf   = texture2D(gFace, probeUV);
    vec3 P    = gf.xyz;
    float vs  = gf.a;                       // voxel edge size -> grazing self-relief skip in the gather
#else
    // Snap the gather origin to a stable world grid (fixed cell N, SAME for every cascade) so the
    // same world cell is sampled each frame -> temporal stability; keep the raw surface for the
    // geometry-aware merge weighting. Outer cascades whose snapped far origin grazes into relief
    // now yield a "started-inside" -> transparent pass-through (see gatherRayWS) instead of the
    // old coherent black bands, so quantizing all levels is safe.
    vec3  P  = quantizeProbePos(Praw, N, c);
    float vs = 0.0;                        // renderer 5: no grazing skip
#endif
