...
SAMPLER2D(histColor, 5);   // FBO9[0] prev: accumulated HDR + age
SAMPLER2D(histPos,   6);   // FBO9[1] prev: world pos for reprojection validation
    ...
    if (frameCount.x > 0.0) {
        ...
        if (valid) {
            ...
            if (moving) {
                ...
                // -- see the fast renderer). A loose distance-scaled position gate catches gross
                // disocclusion only.
