...
// Per-axis coarse forward masks for the skip optimization (replaces the 384-entry
// per-zorder FWD_LO/FWD_HI LUTs). For a 4x4x4 Z-order (bits: Z1,Y1,X1,Z0,Y0,X0),
// each axis has a lower-half mask (axis coord 0-1) and an upper-half mask (coord 2-3).
// Only 6 masks total (2 per axis) vs. the previous 768.
// Index: axis*2 + half   axis: 0=X,1=Y,2=Z   half: 0=LO,1=HI
//
// IMPORTANT: The 32-bit values below use the REVERSED bit ordering of
// checkZOrderInValidMasks: bit 31 = z-order 0, bit 0 = z-order 31 (within each word).
// z-orders 32-63 go to FWD_MASK_HI with the same reversal. This means, for example,
// z-orders 0-7 (the first block of 8) occupy bits 31-24 = 0xFF000000, not 0x000000FF.
...
// no pruning (identity mask = 0xFFFFFFFF). Replaces the 384-entry per-zorder LUT
// with 6 constant masks selected by the current z-order's high bits.
bool anyValidAhead(uint mask1, uint mask2, uint zOrder, ivec3 stepDir) {
    uint fwdLo = 0xFFFFFFFFu;
    uint fwdHi = 0xFFFFFFFFu;

    // X axis: z-order bit 3 = X1 (X coordinate >= 2 when set)
    ...
    // Y axis: z-order bit 4 = Y1 (Y coordinate >= 2 when set)
    ...
    // Z axis: z-order bit 5 = Z1 (Z coordinate >= 2 when set)
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (cellExitT > rayT) {
                ...
                if (!anyValidAhead(data.data1, data.data2,
                    ...
                    // proposedTraversalPosition mirrors the normal DDA step: one node-width
                    // past the exit face. traversalPosition itself stays UNCHANGED, exactly
                    // like the normal pop path keeps it at the pre-step value.
                    ...
                    // getBoundariesCrossed assumes 1-step deltas and may miss the child-node
                    // exit when boundaryPos isn't 4-aligned. We KNOW we exited the child, so
                    // at minimum 1 boundary was crossed; take the max.
                    if (boundariesCrossed < 1u) boundariesCrossed = 1u;

                    if (boundariesCrossed >= nodeStackQuantity) {
                        // Would pop the root — the ray has exited the entire volume.
                        returnData.foundBox.position = vec3(0);
                        returnData.foundBox.size = -1;
                        returnData.steps = stepCount;
                        return returnData;
                    ...
                    // Pop boundariesCrossed levels — identical to the normal DDA pop path.
                    nodeStackQuantity -= boundariesCrossed;
                    candidateNodeLevel += boundariesCrossed;
                    shift = 2u * candidateNodeLevel;
                    stepSize = 1u << (shift);
                    // NORMAL POP keeps traversalPosition at its pre-step value, then
                    // re-rounds to the new (coarser) level. Do the same.
                    traversalPosition = (traversalPosition >> shift) << shift;
                    previouslyPopped = true;
                    data = tree64(nodeStack[nodeStackQuantity - 2u].dataIndex);

                    // The Z-order on the (now-top) stack entry was set for the old finer
                    // level's cell. Recompute it for the new position at the new level,
                    // exactly like the normal pop path implicitly does via getZOrderInParent.
                    nodeStack[nodeStackQuantity - 1u].thisNodeZOrderInParent =
                        getZOrderInParentFromThisNodesLevel(traversalPosition, candidateNodeLevel);

                    // If we popped more than 1 level, the intermediate stack entries are
                    // stale. Check the parent's forward mask too: if nothing is ahead at
                    // this coarser level either, keep popping. This mirrors what the normal
                    // DDA would do (step through the parent and pop again).
                    while (previouslyPopped &&
                           nodeStackQuantity > 2u &&
                           !checkZOrderInValidMasks(data.data1, data.data2, nodeStack[nodeStackQuantity - 1u].thisNodeZOrderInParent) &&
                           !anyValidAhead(data.data1, data.data2, nodeStack[nodeStackQuantity - 1u].thisNodeZOrderInParent, stepI)) {
                        // Pop one more level
