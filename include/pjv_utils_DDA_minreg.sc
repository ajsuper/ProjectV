...
// nodeStack and nodeStackQuantity moved to marchRayThroughTree64_DDA locals
...
SceneIntersectData makeHitResult(ivec3 pos, uint size, uint steps, float t, vec3 nrm) {
    SceneIntersectData r;
    r.foundBox.position = vec3(pos);
    r.foundBox.size = float(size);
    r.steps = steps;
    r.rayT = t;
    r.normal = nrm;
    return r;
}
SceneIntersectData makeMissResult(uint steps) {
    SceneIntersectData r;
    r.foundBox.position = vec3(0);
    r.foundBox.size = -1.0;
    r.steps = steps;
    r.rayT = -1.0;
    r.normal = vec3(0.0);
...
SceneIntersectData marchRayThroughTree64_DDA(Ray ray, RayQuery rayQuery, BoxAABB boundingBox, uint tree64StartIndex, uint tree64Resolution) {
    CombinedNode64 nodeStack[5];
    uint nodeStackQuantity = 0;
    uint candidateNodeLevel = uint(log2(tree64Resolution)/2.0) - 1u;

    uint shift = 2u * candidateNodeLevel;
    vec3 hitNormal = getRayBoxEntry(ray, boundingBox).normal;

    ivec3 traversalPosition = determineTraversalCoordinatesFromRayAndBox(ray, boundingBox);
    ...
    for (; stepCount < rayQuery.maxRaySteps; stepCount++) {
        ...
        while (candidateNodeLevel >= 0) {
            ...
            if (checkZOrderInValidMasks(data.data1, data.data2, nodeStack[nodeStackQuantity - 1u].thisNodeZOrderInParent)) {
                if((data.data3 & 0b1) == 1u) {
                    return makeHitResult(traversalPosition, 1u << shift, stepCount, rayT, hitNormal);
                ...
                candidateBox.size = float(1u << shift);
                ...
                //traversalPosition = roundVoxelCoordinatesBasedOnNodeSize(highResPosition, (1u << shift));
        ...
        vec3 nextPlanes = mix(vec3(traversalPosition), vec3(traversalPosition) + vec3(1u << shift), step(vec3(0.0), ray.direction));

        vec3 tMax = (nextPlanes - ray.origin) * (1.0/ray.direction);
        vec3 tDelta = (1u << shift) * abs(1.0/ray.direction);
        ...
        for (int i = 0; i < 12; i++) {
            ...
            ivec3 proposedTraversalPosition = traversalPosition + directionSteppedIn * (1u << shift);
            if (proposedTraversalPosition.x < 0 || proposedTraversalPosition.y < 0 || proposedTraversalPosition.z < 0 ||
              proposedTraversalPosition.x >= tree64Resolution || proposedTraversalPosition.y >= tree64Resolution || proposedTraversalPosition.z >= tree64Resolution) {
                return makeMissResult(stepCount);
            }
            ...
            if (boundariesCrossed != 0) { // If we cross boundaires, pop.
                ...
                //traversalPosition = roundVoxelCoordinatesBasedOnNodeSize(traversalPosition, (1u << shift));
                traversalPosition = (traversalPosition >> shift) << shift;
...
SceneIntersectData castRayThroughTree64(Ray ray, RayQuery rayQuery, uint headerIndex) {
    ...
    if(rootIntersect.distance >= 0){
        tree64Intersect = marchRayThroughTree64_DDA(transformedRay, rayQuery, tree64BoundingBox, tree64StartIndex, header.resolution);
    }
